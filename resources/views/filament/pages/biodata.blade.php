<x-filament-panels::page>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <img 
                            ...
                            x-on:click="$dispatch('open-modal', { 
                                id: 'image-modal', 
                                image: '{{ $user->image ? asset('storage/' . $user->image) : asset('images/default-user.png') }}' 
                            })"
                        >
                    </div>
                    ...
                    <div>
                        ...
                        <img 
                            src="{{ $user->scanijazah ? asset('storage/' . $user->scanijazah) : asset('images/default-ijazah.png') }}" 
                            alt="Scan Ijazah"
                            class="w-32 h-auto object-cover rounded-md border cursor-pointer"
                            x-on:click="$dispatch('open-modal', { 
                                id: 'image-modal', 
                                image: '{{ $user->scanijazah ? asset('storage/' . $user->scanijazah) : asset('images/default-ijazah.png') }}' })">
                    </div>
            </div>
        </div>
    </x-filament::section>
    ...
    <x-filament::modal id="image-modal">
        <div 
            class="p-4 flex items-center justify-center overflow-hidden max-h-screen"
            x-data="{ 
                image: '', 
                scale: 1, 
                offsetX: 0, 
        ...
        >
            <div 
                class="relative max-w-full max-h-screen overflow-auto" 
            ...
            >
                ...
                <img 
                    x-bind:src="image" 
                    alt="Preview" 
                    class="rounded-md object-contain cursor-grab"
                    :style="'transform: scale(' + scale + ') translate(' + offsetX + 'px, ' + offsetY + 'px)'"
                    x-on:mousedown.prevent="isDragging = true; startX = $event.clientX; startY = $event.clientY"
